...
class ImageService:
    ...
    @staticmethod
    def get_all_images(page=1, per_page=20, filename=None, tags=None):
        """
        获取所有图片记录（支持分页和条件筛选）

        Args:
            page (int): 页码，默认为 1
            per_page (int): 每页记录数，默认为 20
            filename (str, optional): 按文件名模糊筛选
            tags (str, optional): 按标签筛选
    ...
    @staticmethod
    def upload_image(file, description=None, tags=None, current_user=None):
        """
        上传图片
