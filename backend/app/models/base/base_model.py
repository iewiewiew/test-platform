...
class BaseModel(db.Model):
    ...
    # 注意：is_active、created_by、updated_by、created_at、updated_at 字段
    # 将在表创建时通过事件监听器自动添加到表的末尾
    # 这里先不定义，避免影响子类字段的顺序
...
# 注意：字段顺序优化通过 Alembic 迁移脚本的 env.py 中的 process_revision_directives 函数实现
# 确保字段顺序为：id, [业务字段], is_active, created_by, updated_by, created_at, updated_at
