...
export const useDatabaseInfoStore = defineStore('databaseInfo', {
  ...
  actions: {
     ...
     * 获取数据表数据
     */
    async fetchTableData(databaseName, tableName, page = 1, pageSize = 50, append = false) {
      if (!this.currentConnection) {
        throw new Error('请先选择数据库连接')
      }
      ...
      this.error = null
      try {
        const response = await databaseInfoService.getTableData(
    ...
    async fetchTableStructure(databaseName, tableName) {
      ...
      this.error = null
      try {
        const response = await databaseInfoService.getTableStructure(
          this.currentConnection.id,
          databaseName,
          tableName
        )
    ...
    async executeQuery(query, databaseName = null) {
      ...
      this.error = null
      try {
        const response = await databaseInfoService.executeQuery(
          this.currentConnection.id,
          {
