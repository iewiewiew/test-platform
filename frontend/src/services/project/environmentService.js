...
export const environmentService = {
   ...
   * 环境参数列表分页查询
   */
  async getEnvironmentParameters(environment_id, {
    page = 1,
    per_page = 10,
    search,
  } = {}) {
  ...
  async previewEnvironmentConfig(environment_id) {
    try {
      const [environment, parameters, stats] = await Promise.all([
        this.getEnvironment(environment_id),
        this.getAllEnvironmentParameters(environment_id),
        this.getEnvironmentStats(environment_id),
      ]);
  ...
  async healthCheck(environment_id) {
    try {
      const environment = await this.getEnvironment(environment_id);
      const baseUrl = environment.data.base_url;

      // 简单的健康检查：尝试访问基础URL
      const healthCheckResult = {
        environment_id,
        environment_name: environment.data.name,
        base_url: baseUrl,
        status: 'unknown',
        response_time: null,
