      ...
      <div class="common-search-bar">
        <el-form :inline="true" :model="searchForm" class="demo-form-inline">
          ...
          <el-form-item label="标签">
            <el-input v-model="searchForm.tags" placeholder="请输入标签" clearable @change="handleSearch" />
          </el-form-item>
          <el-form-item>
      ...
      <el-table-column prop="original_filename" label="文件名" min-width="200" show-overflow-tooltip>
        <template #default="{ row }">
          <div class="filename-cell">
            <span class="filename-text" :title="row.original_filename">{{ row.original_filename }}</span>
            <el-button text type="primary" size="small" @click="copyImageUrl(row)" style="margin-left: 8px">复制URL</el-button>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="标签" min-width="150">
        <template #default="{ row }">
          <div v-if="row.tags" class="tags-cell">
            <el-tag v-for="tag in row.tags.split(',')" :key="tag" size="small" style="margin-right: 4px; margin-bottom: 4px">
              {{ tag.trim() }}
            </el-tag>
          </div>
          <span v-else class="text-muted">-</span>
      ...
      <el-form ref="editFormRef" :model="editForm" label-width="80px">
        <el-form-item label="更新图片">
          <el-upload :auto-upload="false" :show-file-list="true" :limit="1" accept="image/*" :on-change="handleFileChange" :on-remove="handleFileRemove" :before-upload="beforeUpload">
            ...
            <template #tip>
              <div class="el-upload__tip">选择新图片将替换当前图片，但保持 UUID 不变（URL 不变）</div>
            </template>
          </el-upload>
        </el-form-item>
...
const handleSearch = async () => {
  try {
    await store.fetchImages({
      ...
      tags: searchForm.value.tags || undefined
    })
  } catch (error) {
