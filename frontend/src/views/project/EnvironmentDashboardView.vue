  ...
  <div class="common-list-container">
    <div class="common-header-bar">
      <div class="common-action-bar">
        ...
        <span v-if="loading && metrics.length === 0" class="loading-tip">
          正在请求接口，请耐心等待...
        </span>
      </div>
    </div>
      ...
      >
        ...
        <div class="metrics metrics-circle">
          <div class="metric-circle">
            <el-progress 
              type="dashboard" 
              :percentage="safePct(item.cpu_usage)" 
              :status="circleStatus(item.cpu_usage)"
              :width="110"
            />
          ...
          <div class="metric-circle">
            <el-progress 
              type="dashboard" 
              :percentage="safePct(item.memory_usage)" 
              :status="circleStatus(item.memory_usage)"
              :width="110"
            />
          ...
          <div class="metric-circle">
            <el-progress 
              type="dashboard" 
              :percentage="safePct(item.disk_usage)" 
              :status="circleStatus(item.disk_usage)"
              :width="110"
            />
...
.usage-low { color: #52c41a; }
.usage-medium { color: #faad14; font-weight: 600; }
.usage-high { color: #ff4d4f; font-weight: 600; }
.usage-unknown { color: #909399; }
.extras {
  margin-top: 10px;
  font-size: 12px;
  color: #606266;
}
.extra-item { display: flex; justify-content: space-between; }
.extra-label { color: #909399; }
.load-badges > .el-tag { margin-right: 6px; }
</style>
